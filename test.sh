...
killservers()
{
    kill $(ps aux | grep '[P]ython Test/servers' | awk '{print $2}') > /dev/null 2>&1
}
killservers
python3 Test/servers/server-test1.py > /dev/null 2>&1 &
sleep 3
...
killservers
python3 Test/servers/server-test2.py > /dev/null 2>&1 &
sleep 5

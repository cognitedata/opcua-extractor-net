...
namespace Test.Unit
{
    ...
    public class FDMTests : IClassFixture<FDMTestFixture>
    {
        ...
        public async Task TestMapCustomTypes()
        {
            ...
            Assert.Equal(11, handler.Containers.Count);
            foreach (var inst in handler.Instances)
            {
                tester.Log.LogInformation("{Key}", inst.Key);
            }
            Assert.Equal(54, handler.Instances.Count(inst => inst.Value["instanceType"].ToString() == "node"));
        ...
        public async Task TestMapReferencedTypesNoTrim()
        {
            ...
            // 8 base types, 2 custom object types, 1 custom variable type have container data
            Assert.Equal(11, handler.Containers.Count);
            Assert.Equal(2143, handler.Instances.Count(inst => inst.Value["instanceType"].ToString() == "node"));
        ...
        public async Task TestMapEverything()
        {
            ...
            Assert.Single(handler.Spaces);
            Assert.Equal(212, handler.Views.Count);
            Assert.Equal(131, handler.Containers.Count);
            ...
            Assert.Equal(1667, handler.Instances.Count(inst => inst.Value["instanceType"].ToString() == "node"));

...
namespace OpcUaService
{
    public partial class OpcUaService : ServiceBase
    {
        ...
        private static string[] _extractorArgs;
        ...
        public OpcUaService(string[] args)
        {
            ...
            _args = args;
            _numberOfFailures = 0;
            _extractorArgs = Array.Empty<string>();
        }
        ...
        protected override void OnStart(string[] args)
        {
            ...
            if (_args.Length > 0)
            {
                ...
                {
                    LogMessage($"Provided path does not exist. {_path}", EventLogEntryType.Error);
                    Stop();
                    Environment.Exit(1);
                }
            ...
            //_configFile = string.Format(@"{0}\{1}", _path, _configFileName);
            _configFile = $@"{_path}\{_configFileName}";

            // TODO: Add a function to validate config file here and stop service and log error
            //string configFileTest = PiExtractor.Program.ValidateConfigurationFile(_configFile);

            //if (configFileTest != "OK")
            if (!File.Exists(_configFile))
            {
                LogMessage($"Configuration file not found. {_configFile}", EventLogEntryType.Error);
                Stop();
                Environment.Exit(1);
            }
            ...
            _extractorThread = new Thread(new ThreadStart(() => Cognite.OpcUa.Program.Start(_extractorArgs)));
        ...
        protected override void OnStop()
        {
            ...
            if (Cognite.OpcUa.Program.RunningStatus())
            {
                Cognite.OpcUa.Program.Stop();
            }

            if (_extractorThread != null)
            {
                while (Cognite.OpcUa.Program.RunningStatus() || _extractorThread.IsAlive)
                {
                    Thread.Sleep(500);
                }
            }
        ...
        private void CheckExtractorStatus(object sender, ElapsedEventArgs e)
        {
            ...
            try
            {
                ...
                bool isExtractorRunning = Cognite.OpcUa.Program.RunningStatus();
        ...
        private void LogMessage(string message, EventLogEntryType level)
        {
            ...
            try
            {
                ...
                //File.AppendAllText(@"c:\tmp\ext.log", message + "\r\n");
            }
            catch (Exception ex)
            {

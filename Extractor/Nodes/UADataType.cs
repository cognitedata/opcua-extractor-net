...
namespace Cognite.OpcUa.Nodes
{
    ...
    public class UADataType : BaseUAType
    {
        ...
        public UADataPoint ToDataPoint(IUAClientAccess client, object value, DateTime timestamp, string id, StatusCode status, bool stringOverride = false, DateTime? receivedTimestamp = null, uint? sequenceNumber = null)
        {
            if (timestamp == DateTime.MinValue) timestamp = DateTime.UtcNow;
            if (value is null)
            {
                return new UADataPoint(timestamp, id, IsString, status, receivedTimestamp, sequenceNumber);
            }

            if (IsString || stringOverride)
            {
                return new UADataPoint(timestamp, id, client.StringConverter.ConvertToString(value, EnumValues), status, receivedTimestamp, sequenceNumber);
            }
            return new UADataPoint(timestamp, id, UAClient.ConvertToDouble(value), status, receivedTimestamp, sequenceNumber);
        }

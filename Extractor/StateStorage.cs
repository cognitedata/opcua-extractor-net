...
namespace Cognite.OpcUa
{
    public sealed class StateStorage : IDisposable
    {
        ...
        public StateStorage(Extractor extractor, FullConfig config)
        {
            ...
            {
                var stringDataQueue = new LiteQueue<StringDataPointPoco>(db, StringDPQueue);
                stringDataQueue.ResetOrphans();
                var doubleDataQueue = new LiteQueue<DoubleDataPointPoco>(db, DoubleDPQueue);
                doubleDataQueue.ResetOrphans();
                var eventQueue = new LiteQueue<EventPoco>(db, EventQueue);
